...
def github_commit_push(filename, message):
    if not GITHUB_TOKEN or not REPO_NAME:
        log("⚠️ GitHub token or repo missing, skipping auto-push")
        return
    try:
        # Prepare API URL
        ...
        if r.status_code in [200, 201]:
            ...
            log(f"❌ GitHub push failed: {r.status_code} {r.text}")
    except Exception as e:
...
def save_all(memory, tasks, upgrades, apps, metrics):
    ...
    for f in files:
        save_json(f, locals()[f.split(".")[0]])
        github_commit_push(f, f"Auto-update: {f} from VP run")
