...
REPO_NAME = os.getenv("GITHUB_REPO")  # e.g., "username/VicePresidentAI"
...
def load_json(file):
    try:
        ...
        if "memory" in file: return {"last_update": None, "poe_chat_id": None}
        elif "tasks" in file: return {"tasks": []}
        elif "upgrades" in file: return {"upgrades": []}
        elif "apps" in file: return {"apps": []}
        return {}
...
# Commit file to GitHub automatically
...
# Process upgrade via Poe and store output in apps.json
def process_upgrade(upgrade, conversation_id):
    ...
    # Save as app
    apps_data = load_json(APP_FILE)
    app_entry = {
        "id": len(apps_data.get("apps", [])) + 1,
        "description": upgrade["description"],
        "output": output_text,
        "timestamp": str(datetime.now())
    }
    apps_data.setdefault("apps", []).append(app_entry)
    save_json(APP_FILE, apps_data)
    commit_to_github(APP_FILE, f"Add app output {app_entry['id']}")
    upgrade["status"] = "completed"
    upgrade["completed_at"] = str(datetime.now())
    upgrade["result"] = f"Stored in apps.json as ID {app_entry['id']}"
    log(f"Upgrade stored: {upgrade['result']}")
...
# Adapt priorities for remaining upgrades
...
# Main loop
def main():
    log("Vice President adaptive app cycle start.")
    ...
    # Commit upgrades
    commit_to_github(UPGRADE_FILE, "Update upgrade statuses")
    ...
    log("Vice President adaptive app cycle complete.")
