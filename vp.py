...
def load_json(file, default):
    ...
    try:
        with open(file, "r") as f:
            data = json.load(f)
            if data is None:
                log(f"⚠️ {file} empty, using default")
                return default
            return data
...
def ensure_keys(memory):
    ...
    for k, v in required_keys.items():
        if k not in memory:
            log(f"⚠️ Key '{k}' missing in memory, initializing")
            memory[k] = v
    return memory
...
def vp_cycle(memory, apps, tasks, upgrades, metrics):
    ...
    for t in tasks:
        if not t.get("completed", False):
            ...
            completed_apps += 1
    log(f"📌 Completed {completed_apps} tasks, total apps: {memory['apps']}")
...
# --- Main ---
def main():
    memory = load_json(MEMORY_FILE, None)
    if memory is None:
        memory = {}
